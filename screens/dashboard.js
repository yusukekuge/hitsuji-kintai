  ...
  async function render() {
    ...
    for (const s of staff) {
      ...
      dates.forEach(date => {
        ...
        if (dw.isComplete) {
          totalWork += dw.workMinutes;
          totalNight += dw.nightMinutes;
          workDays++;
        }
      });
        ...
        ${summaries.map(sum => `
          <div class="summary-card">
            ...
            <div class="flex-between mb-8">
              <span>出勤日数</span>
              <span class="value" style="font-size:1.2rem;">${sum.workDays}日</span>
            </div>
            <div class="flex-between mb-8">
              <span>総労働時間</span>
              <span>${Utils.minutesToHMJP(sum.totalWork)}</span>
            </div>
            <div class="flex-between mb-8">
              <span>深夜時間</span>
              <span>${Utils.minutesToHMJP(sum.totalNight)}</span>
            </div>
            <div class="flex-between mb-8">
              <span>年間累計</span>
              <span>${Utils.formatCurrency(sum.annualTotal)}</span>
            </div>
            ${sum.warnings.map(w => `
              <div class="alert-box alert-${w.type}">
                <strong>${w.line}</strong> ${w.message}
              </div>
            `).join('')}
          </div>
        `).join('')}
