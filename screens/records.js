...
const RecordsScreen = (() => {
  ...
  async function render() {
    ...
    const typeLabels = {
      clock_in: '出勤', break_start: '休憩開始',
      break_end: '休憩終了', clock_out: '退勤'
    };
  ...
  async function editRecord(staffId, date) {
    ...
    msgEl.querySelectorAll('.edit-save-btn').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        const confirmed = confirm(`${typeName}の打刻時間を ${timeVal} に修正しますか？`);
        if (!confirmed) return;
    ...
    msgEl.querySelectorAll('.edit-delete-btn').forEach(btn => {
      btn.addEventListener('click', async () => {
        const typeName = typeLabels[btn.dataset.type] || btn.dataset.type;
        const confirmed = confirm(`${typeName}の打刻を削除しますか？\nこの操作は元に戻せません。`);
        if (!confirmed) return;
    ...
    const closeModal = () => { modal.classList.remove('active'); render(); };
