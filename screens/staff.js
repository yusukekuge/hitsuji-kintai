...
const StaffScreen = (() => {
  ...
  async function render() {
    ...
    container.innerHTML = `
      <div class="card">
        ...
        ${activeStaff.length >= 5 ? '<div class="alert-box alert-warning mb-16">スタッフ登録上限（5人）に達しています</div>' : ''}
      </div>
        ...
        <form id="staff-form">
          <div class="form-row">
            <div class="form-group">
              ...
              <input type="text" class="form-input" id="sf-name" required maxlength="20">
            </div>
            <div class="form-group">
              <label class="form-label">時給（円）</label>
              <input type="number" class="form-input" id="sf-wage" value="1150" min="0" max="5000">
            </div>
          </div>
          <div class="form-row">
            <div class="form-group">
              <label class="form-label">通勤距離（km）</label>
              <input type="number" class="form-input" id="sf-distance" value="0" min="0" max="100" step="0.1">
            </div>
            <div class="form-group">
              <label class="form-label">入社日</label>
              <input type="date" class="form-input" id="sf-hire-date">
            </div>
          </div>
          <div class="form-row">
            <div class="form-group">
              <label class="form-label">その他手当（円/月）</label>
              <input type="number" class="form-input" id="sf-other" value="0" min="0">
            </div>
            <div class="form-group">
              <label class="form-label">試用期間</label>
              <select class="form-select" id="sf-probation">
                <option value="false">なし</option>
                <option value="true">試用期間中</option>
              </select>
            </div>
          </div>
          <div class="form-row">
            <div class="form-group">
              <label class="form-label">源泉徴収区分</label>
              <select class="form-select" id="sf-tax-category">
                <option value="kou">甲欄（主たる給与）</option>
                <option value="otsu">乙欄（従たる給与）</option>
              </select>
            </div>
            <div class="form-group" id="sf-dependents-group">
              <label class="form-label">扶養人数（甲欄の場合）</label>
  ...
  async function handleSubmit(e) {
    ...
    if (!editingId && activeStaff.length >= 5) {
      Utils.showToast('スタッフ登録上限（5人）に達しています', 'error');
      return;
    }
